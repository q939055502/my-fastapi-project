...
from src.schemas.iam.user import UserCreate
...
class AuthService:
    ...
    def login_step1(self, credentials: LoginRequest) -> dict[str, Any]:
        ...
        with TransactionManager() as tm:
            ...
            # 获取用户的所有租户信息
            tenant_memberships = tenant_member_repository.get_user_tenants(user.id, session=tm.session)
            ...
            for idx, tenant in enumerate(tenant_memberships):
                ...
                # 这里需要调整一下 tenant_member_repository 的 get_user_tenants 返回更多信息
