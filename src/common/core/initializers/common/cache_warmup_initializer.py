...
def _warmup_system_config() -> int:
    ...
    for session in get_db():
        ...
        for config in configs:
            ...
            cache_manager.set(cache_key, cache_value, l1_ttl=settings.L1_CACHE_TTL_LOW, l2_ttl=settings.L2_CACHE_TTL_LOW)
            cache_count += 1
...
def _warmup_dict_data() -> int:
    ...
    for session in get_db():
        ...
        for dict_type in dict_types:
            ...
            cache_key = f"dict_data:{dict_type.code}"
            cache_manager.set(cache_key, data_list, l1_ttl=settings.L1_CACHE_TTL_LOW, l2_ttl=settings.L2_CACHE_TTL_LOW)
            cache_count += 1

        cache_manager.set("dict_type:all", type_list, l1_ttl=settings.L1_CACHE_TTL_LOW, l2_ttl=settings.L2_CACHE_TTL_LOW)
        cache_count += 1
...
def _warmup_permission_data() -> int:
    ...
    for session in get_db():
        ...
        for perm in permissions:
            ...
            cache_key = f"permission:{perm.id}"
            cache_manager.set(cache_key, perm_list[-1], l1_ttl=settings.L1_CACHE_TTL_LOW, l2_ttl=settings.L2_CACHE_TTL_LOW)
            cache_count += 1

        cache_manager.set("permission:all", perm_list, l1_ttl=settings.L1_CACHE_TTL_LOW, l2_ttl=settings.L2_CACHE_TTL_LOW)
        cache_count += 1
...
def _warmup_role_data() -> int:
    ...
    for session in get_db():
        ...
        for role in roles:
            ...
            cache_manager.set(cache_key, role_cache, l1_ttl=settings.L1_CACHE_TTL_LOW, l2_ttl=settings.L2_CACHE_TTL_LOW)
            cache_count += 1
            ...
            perm_cache_key = f"role:{role.id}:permissions"
            cache_manager.set(perm_cache_key, permissions, l1_ttl=settings.L1_CACHE_TTL_LOW, l2_ttl=settings.L2_CACHE_TTL_LOW)
            cache_count += 1

        cache_manager.set("role:all", role_list, l1_ttl=settings.L1_CACHE_TTL_LOW, l2_ttl=settings.L2_CACHE_TTL_LOW)
        cache_count += 1
...
def _warmup_tenant_data() -> int:
    ...
    for session in get_db():
        ...
        for tenant in tenants:
            ...
            cache_manager.set(cache_key, tenant_cache, l1_ttl=settings.L1_CACHE_TTL_LOW, l2_ttl=settings.L2_CACHE_TTL_LOW)
            cache_count += 1

        cache_manager.set("tenant:all", tenant_list, l1_ttl=settings.L1_CACHE_TTL_LOW, l2_ttl=settings.L2_CACHE_TTL_LOW)
        cache_count += 1
...
def _warmup_user_data() -> int:
    ...
    for session in get_db():
        ...
        for user in users:
            ...
            cache_manager.set(cache_key, user_cache, l1_ttl=settings.L1_CACHE_TTL_MEDIUM, l2_ttl=settings.L2_CACHE_TTL_MEDIUM)
            cache_count += 1

        cache_manager.set("user:all", user_list, l1_ttl=settings.L1_CACHE_TTL_LOW, l2_ttl=settings.L2_CACHE_TTL_LOW)
        cache_count += 1
...
def _warmup_account_bind_data() -> int:
    ...
    for session in get_db():
        ...
        for user_id, bind_list in account_bind_dict.items():
            cache_key = f"account_bind:{user_id}"
            cache_manager.set(cache_key, bind_list, l1_ttl=settings.L1_CACHE_TTL_LOW, l2_ttl=settings.L2_CACHE_TTL_LOW)
            cache_count += 1

        # 缓存所有绑定关系
        cache_manager.set("account_bind:all", all_bind_list, l1_ttl=settings.L1_CACHE_TTL_LOW, l2_ttl=settings.L2_CACHE_TTL_LOW)
        cache_count += 1
