...
class Member(BaseModel, TimestampMixin, SoftDeleteMixin, RemarkMixin, EnableStatusMixin, UUIDModel):
    ...
    created_by_member_id = Column(BigInteger, nullable=True, index=True, comment="创建者成员ID")
    ...
    __table_args__ = (
        UniqueConstraint('user_id', 'tenant_id', name='uq_platform_member'),
        UniqueConstraint('subject_id', name='uq_platform_member_subject_id'),
    )
