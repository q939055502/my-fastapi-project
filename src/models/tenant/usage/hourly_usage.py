...
class HourlyUsage(BaseModel, TimestampMixin):
    ...
    不参与核心配额校验,核心校验�?platform_usage(月度表�?    """
    ...
    storage_delta = Column(BigInteger, default=0, comment="存储变更�?MB)�?新增 / -删除")
    storage_total = Column(BigInteger, default=0, comment="当前累计存储总量(MB)")

    bandwidth_delta = Column(BigInteger, default=0, comment="带宽小时消�?GB)")
