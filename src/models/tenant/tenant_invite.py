...
class TenantInvite(BaseModel, TimestampMixin, SoftDeleteMixin, RemarkMixin):
    ...
    default_role_id = Column(BigInteger, ForeignKey("tenant_role.id"), nullable=True, comment="默认角色ID")
    need_audit = Column(Integer, default=0, comment="是否需要审批：0无需审批 1需要审批")
    status = Column(Integer, default=1, comment="状态：1有效 0禁用")
