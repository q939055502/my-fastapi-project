...
class OrderRefund(BaseModel, TimestampMixin, SoftDeleteMixin, RemarkMixin):
    ...
    order_payment_id = Column(Integer, ForeignKey("order_payment.id"), nullable=True, index=True, comment="支付记录ID")
    ...
    operator_id = Column(Integer, nullable=True, index=True, comment="操作人ID")
