...
class UserAdminService:
    ...
    def get_user_list(
        ...
        username: str = "",
        email: str = "",
        dept_uuid: UUID | None = None,
    ) -> tuple[int, list[dict]]:
        """获取用户列表（管理员）"""
        with TransactionManager() as tm:
            search_filters = self._build_user_search_filters(
                username=username, email=email, dept_uuid=dept_uuid, session=tm.session
            )
    ...
    def create_user(self, user_in: UserCreate) -> dict:
        ...
        with TransactionManager() as tm:
            ...
            try:
                ...
                user_repository.update_roles(new_user, role_uuids_to_assign, session=tm.session)
    ...
    def update_user(self, user_uuid: UUID, user_in: UserUpdate) -> None:
        ...
        with TransactionManager() as tm:
            ...
            if user_in.role_uuids is not None:
                ...
                user_repository.update_roles(user, user_in.role_uuids, session=tm.session)
    ...
    def _build_user_search_filters(
        ...
        if dept_uuid is not None:
            dept = dept_repository.get_by_uuid(uuid=dept_uuid, session=session)
            if dept:
                filters.append(user_repository.model.dept_id == dept.id)
